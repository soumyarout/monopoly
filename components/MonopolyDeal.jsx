...
const PROPERTY_DECKS = {
  us: {
    ...
    railroads: [
      {name:"Reading RR"}, {name:"Penn RR"}, {name:"B&O RR"}, {name:"Short Line"},
    ],
    utilities: [
      {name:"Electric Co"}, {name:"Water Works"},
    ],
  },
  uk: {
    label: "UK", flag: "🇬🇧",
    properties: [
      {color:"brown",    name:"Old Kent Rd",   value:1},
      {color:"brown",    name:"Whitechapel",   value:1},
      {color:"lightBlue",name:"Angel Isling.", value:1},
      {color:"lightBlue",name:"Euston Road",   value:1},
      {color:"lightBlue",name:"Pentonville",   value:1},
      {color:"pink",     name:"Pall Mall",     value:2},
      {color:"pink",     name:"Whitehall",     value:2},
      {color:"pink",     name:"Northumberland",value:2},
      {color:"orange",   name:"Bow Street",    value:2},
      {color:"orange",   name:"Marlborough",   value:2},
      {color:"orange",   name:"Vine Street",   value:2},
      {color:"red",      name:"The Strand",    value:3},
      {color:"red",      name:"Fleet Street",  value:3},
      {color:"red",      name:"Trafalgar Sq",  value:3},
      {color:"yellow",   name:"Piccadilly",    value:3},
      {color:"yellow",   name:"Coventry St",   value:3},
      {color:"yellow",   name:"Leicester Sq",  value:3},
      {color:"green",    name:"Bond Street",   value:4},
      {color:"green",    name:"Oxford Street", value:4},
      {color:"green",    name:"Regent Street", value:4},
      {color:"blue",     name:"Park Lane",     value:4},
      {color:"blue",     name:"Mayfair",       value:4},
    ],
    railroads: [
      {name:"King's Cross"}, {name:"Marylebone"}, {name:"Fenchurch St"}, {name:"Liverpool St"},
    ],
    utilities: [
      {name:"Electric Co"}, {name:"Water Works"},
    ],
  ...
  india: {
    label: "India", flag: "🇮🇳",
    properties: [
      {color:"brown",    name:"Coimbatore",    value:1},
      {color:"brown",    name:"Madurai",       value:1},
      {color:"lightBlue",name:"Agra",          value:1},
      {color:"lightBlue",name:"Varanasi",      value:1},
      {color:"lightBlue",name:"Patna",         value:1},
      {color:"pink",     name:"Kochi",         value:2},
      {color:"pink",     name:"Surat",         value:2},
      {color:"pink",     name:"Vadodara",      value:2},
      {color:"orange",   name:"Bhopal",        value:2},
      {color:"orange",   name:"Indore",        value:2},
      {color:"orange",   name:"Nagpur",        value:2},
      {color:"red",      name:"Jaipur",        value:3},
      {color:"red",      name:"Chandigarh",    value:3},
      {color:"red",      name:"Lucknow",       value:3},
      {color:"yellow",   name:"Kolkata",       value:3},
      {color:"yellow",   name:"Pune",          value:3},
      {color:"yellow",   name:"Ahmedabad",     value:3},
      {color:"green",    name:"Hyderabad",     value:4},
      {color:"green",    name:"Chennai",       value:4},
      {color:"green",    name:"Bengaluru",     value:4},
      {color:"blue",     name:"Delhi",         value:4},
      {color:"blue",     name:"Mumbai",        value:4},
    ],
    railroads: [
      {name:"Mumbai Central"}, {name:"Delhi Junction"}, {name:"Chennai Central"}, {name:"Howrah Jct"},
    ],
    utilities: [
      {name:"Electric Co"}, {name:"Water Works"},
    ],
...
function createDeck(deckId = "us") {
  let uid = 0;
  const nid = () => String(++uid);
  const cards = [];
  const deck = PROPERTY_DECKS[deckId] || PROPERTY_DECKS.us;
  ...
  [
    ...deck.properties,
    ...deck.railroads.map(r => ({ color:"railroad", value:2, ...r })),
    ...deck.utilities.map(u => ({ color:"utility",  value:2, ...u })),
...
function initGame(lobbyPlayers, deckId = "us") {
  let deck = createDeck(deckId);
...
// ─────────────────────────────────────────────────────────────────────────────
// STORAGE  (proxied through /api/storage — see app/api/storage/route.js)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// RULE-BASED JUDGE  (no API calls needed)
// ─────────────────────────────────────────────────────────────────────────────

const JUDGE_RULES = {
  newGame: (names) => [
    `🎲 Welcome ${names}! Draw 2 cards to start your turn, play up to 3 cards per turn. First to 3 complete sets wins!`,
    `⚖️ Game on, ${names}! Remember: draw 2 to begin, play up to 3 actions, collect 3 full sets to claim victory!`,
  ],
  setComplete: (player, color, total) => [
    `✅ ${player} completes the ${color} set! That's ${total}/3 — the finish line is in sight!`,
    `🏆 ${color} set locked in for ${player}! ${total}/3 complete sets. Watch out!`,
    `🔥 ${player} owns the ${color} set! ${total} of 3 — pressure is mounting!`,
  ],
  doubleRent: (player) => [
    `✖️2 ${player} plays Double Rent! The next rent card hits twice as hard — duck and cover!`,
    `💥 DOUBLED! ${player} is charging double rent next. Everyone's wallet weeps.`,
  ],
  rent: (player, color, amount, doubled) => [
    `💰 ${player} charges $${amount}M ${color} rent${doubled ? " — DOUBLED" : ""}! Pay up!`,
    `🏠 ${player} collects $${amount}M ${color} rent${doubled ? " (×2!)" : ""}. No escaping this one.`,
  ],
  birthday: (player) => [
    `🎂 It's ${player}'s Birthday! Everyone chips in $2M. How generous... or not.`,
    `🎉 ${player} plays Birthday! Fork over $2M each — it's the rules!`,
  ],
  debtCollector: (player, target) => [
    `💼 ${player} sends the Debt Collector after ${target}! Pay $5M or face consequences!`,
    `📋 Debt notice: ${target} owes ${player} $5M. The Debt Collector has spoken.`,
  ],
  justSayNo: (player) => [
    `🚫 ${player} plays Just Say No! Action cancelled — what a counter!`,
    `🛑 BLOCKED! ${player} drops the Just Say No card. Plans ruined!`,
  ],
  slyDeal: (player, card, target) => [
    `🕵️ ${player} swipes ${card} from ${target} with a Sly Deal! Sneaky!`,
    `😈 Sly Deal! ${player} steals ${card} right out of ${target}'s hands!`,
  ],
  dealBreaker: (player, color, target) => [
    `💥 DEAL BREAKER! ${player} steals ${target}'s entire ${color} set! Chaos!`,
    `😱 ${player} smashes the Deal Breaker! The whole ${color} set from ${target} — gone!`,
  ],
  forcedDeal: (player, target) => [
    `🔄 Forced Deal! ${player} and ${target} swap properties. Nobody asked.`,
    `🤝 ${player} forces a swap with ${target}. The deal is done.`,
  ],
  endTurn: (prev, next) => [
    `⏭️ ${prev} ends their turn. ${next}, it's your move — draw 2!`,
    `🎯 ${next}'s turn! Draw 2 cards and make your play.`,
  ],
  illegalSlyDeal: (player, card, target) => [
    `🚨 ILLEGAL MOVE! ${player} cannot Sly Deal ${card} from ${target}'s complete set. Sly Deal only works on incomplete sets!`,
    `⛔ RULED ILLEGAL! Complete sets are protected from Sly Deal. ${player}, pick a different target.`,
  ],
};

function pick(arr) { return arr[Math.floor(Math.random() * arr.length)]; }

function callJudge(situation, gs) {
  const cur = (gs.players||[])[gs.currentPlayerIndex]?.name || "Player";

  if (situation.startsWith("New Monopoly Deal game!")) {
    const names = (gs.players||[]).map(p => p.name).join(", ");
    return pick(JUDGE_RULES.newGame(names));
  }
  if (situation.includes("completed the") && situation.includes("set!")) {
    const m = situation.match(/^(\S+) completed the (\w+) set! That's (\d+)\/3/);
    return pick(JUDGE_RULES.setComplete(m?.[1]||cur, m?.[2]||"", m?.[3]||"?"));
  }
  if (situation.includes("Double the Rent")) {
    return pick(JUDGE_RULES.doubleRent(cur));
  }
  if (situation.includes("rent card for")) {
    const doubled = situation.includes("DOUBLED");
    const m = situation.match(/rent card for (\w+)! \$(\d+)M/);
    return pick(JUDGE_RULES.rent(cur, m?.[1]||"", m?.[2]||"?", doubled));
  }
  if (situation.includes("Birthday")) {
    return pick(JUDGE_RULES.birthday(cur));
  }
  if (situation.includes("Debt Collector")) {
    const m = situation.match(/on (.+)!/);
    return pick(JUDGE_RULES.debtCollector(cur, m?.[1]||"opponent"));
  }
  if (situation.includes("Just Say No")) {
    return pick(JUDGE_RULES.justSayNo(cur));
  }
  if (situation.includes("ILLEGAL MOVE") && situation.includes("Sly Deal")) {
    const m = situation.match(/Sly Deal (.+) from (.+)'s COMPLETE/);
    return pick(JUDGE_RULES.illegalSlyDeal(cur, m?.[1]||"property", m?.[2]||"opponent"));
  }
  if (situation.includes("Sly Deal")) {
    const m = situation.match(/swipe (.+) from (.+)!/);
    return pick(JUDGE_RULES.slyDeal(cur, m?.[1]||"property", m?.[2]||"opponent"));
  }
  if (situation.includes("DEAL BREAKER")) {
    const m = situation.match(/stole (.+)'s entire (\w+) set/);
    return pick(JUDGE_RULES.dealBreaker(cur, m?.[2]||"", m?.[1]||"opponent"));
  }
  if (situation.includes("Forced Deal")) {
    const m = situation.match(/swap with (.+)!/);
    return pick(JUDGE_RULES.forcedDeal(cur, m?.[1]||"opponent"));
  }
  if (situation.includes("ended their turn")) {
    const m = situation.match(/^(.+) ended their turn\. It's (.+)'s turn/);
    return pick(JUDGE_RULES.endTurn(m?.[1]||cur, m?.[2]||"next player"));
  }
  return `⚖️ ${cur}'s turn — play wisely!`;
}
...
function LobbyScreen({ onCreate, onJoin }) {
  ...
  const [deckId, setDeckId] = useState("us");
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ color:"#555", fontSize:11, marginBottom:16 }}>
          Multiplayer · Live Judge · Collect 3 complete sets to win!
        </p>

        {/* Deck / Edition selector */}
        <div style={{ marginBottom:16 }}>
          <div style={{ fontSize:10, color:"#555", marginBottom:6 }}>Choose Edition</div>
          <div style={{ display:"flex", gap:6, justifyContent:"center" }}>
            {Object.entries(PROPERTY_DECKS).map(([id, d]) => (
              <button
                key={id}
                onClick={() => setDeckId(id)}
                style={{
                  background: deckId === id ? "#2a7a2a" : "rgba(255,255,255,0.05)",
                  border: `2px solid ${deckId === id ? "#FFD700" : "#2a5a2a"}`,
                  borderRadius:8, padding:"8px 14px", cursor:"pointer",
                  color: deckId === id ? "#FFD700" : "#888",
        ...
        <input
          ...
          onKeyDown={e => e.key === "Enter" && run(() => onCreate(name, deckId))}
          style={{ ...inputStyle, marginBottom:12 }}
        />
        <button onClick={() => run(() => onCreate(name, deckId))} disabled={busy} style={bigBtn}>
          {busy ? "..." : "🏠 Create Room"}
        </button>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ color:"#444", fontSize:11, marginBottom:4 }}>Share this code with friends!</p>
        {lobby?.deckId && (
          <p style={{ color:"#666", fontSize:11, marginBottom:16 }}>
            Edition: {PROPERTY_DECKS[lobby.deckId]?.flag} {PROPERTY_DECKS[lobby.deckId]?.label}
          </p>
        )}
...
export default function App() {
  ...
  const [judgeMsg, setJudgeMsg] = useState("🎩 Welcome to Monopoly Deal! I'm your live judge.");
  ...
  // ── LOBBY ──────────────────────────────────────────────────────────────────
  const onCreateRoom = async (name, deckId = "us") => {
    if (!name.trim()) return "Enter your name";
    const code = Math.random().toString(36).slice(2,7).toUpperCase();
    const l = { code, deckId, players:[{ id:pid, name:name.trim(), isHost:true }], gameStarted:false };
  ...
  const onStart = async () => {
    const g = initGame(lobby.players, lobby.deckId || "us");
